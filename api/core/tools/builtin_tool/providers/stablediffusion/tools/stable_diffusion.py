        ...
        if len(self.list_default_image_variables()) != 0:
            parameters.append(
                ToolParameter(
                    ...
                    required=True,
                    options=[
                        PluginParameterOption(value=i.name, label=I18nObject(en_US=i.name, zh_Hans=i.name))
            ...
            if len(sample_methods) != 0:
                parameters.append(
                    ToolParameter(
                        ...
                        options=[
                            PluginParameterOption(value=i, label=I18nObject(en_US=i, zh_Hans=i)) for i in sample_methods
                        ],
                    )
                )
        return parameters
