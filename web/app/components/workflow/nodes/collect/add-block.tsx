...
type AddBlockProps = {
  ...
  collectNodeData: CollectNodeType
}
const AddBlock = ({
  collectNodeId,
  collectNodeData,
  ...
  const { availablePrevBlocks } = useAvailableBlocks(collectNodeData.startNodeType, true)
  ...
  const handleSelect = useCallback<OnSelectBlock>((type, toolDefaultValue) => {
    ...
    const newNode = generateNewNode({
      ...
      zIndex: 1002,
      parentId: collectNodeId,
      extent: 'parent',
    })
    const newNodes = produce(nodes, (draft) => {
      draft.forEach((node) => {
        if (node.id === collectNodeId) {
  ...
  }, [store, t, collectNodeId, saveStateToHistory])
  ...
  return (
    <div className='absolute top-12 left-6 flex items-center h-8 z-10'>
      <TooltipPlus popupContent={t('workflow.blocks.collect-start')}>
      ...
      <div className='group/insert relative w-16 h-0.5 bg-gray-300'>
        {
          collectNodeData.startNodeType && (
            <InsertBlock
              startNodeId={collectNodeData.start_node_id}
