...
export const useNodeCollectInteractions = () => {
  ...
  const handleNodeCollectRerender = useCallback((nodeId: string) => {
    const {
      getNodes,
      setNodes,
    } = store.getState()
  ...
  const handleNodeCollectChildDrag = useCallback((node: Node) => {
  ...
  const handleNodeCollectChildSizeChange = useCallback((nodeId: string) => {
    const { getNodes } = store.getState()
    const nodes = getNodes()
    const currentNode = nodes.find(n => n.id === nodeId)!
    ...
    if (parentId)
      handleNodeCollectRerender(parentId)
  }, [store, handleNodeCollectRerender])

  const handleNodeCollectChildrenCopy = useCallback((nodeId: string, newNodeId: string) => {
    const { getNodes } = store.getState()
    const nodes = getNodes()
    const childrenNodes = nodes.filter(n => n.parentId === nodeId)
  ...
  return {
    handleNodeCollectRerender,
    handleNodeCollectChildDrag,
    handleNodeCollectChildSizeChange,
    handleNodeCollectChildrenCopy,
  }
}
